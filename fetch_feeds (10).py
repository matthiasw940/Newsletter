#!/usr/bin/env python3
"""Morning Brief v12 — AI briefing, weather, court summaries."""
...
def fetch_decision_text(url):
    """Fetch the full text of a court decision from its RIS URL."""
    if not url or "ris.bka.gv.at" not in url:return ""
    try:
        from bs4 import BeautifulSoup
        html=http_get(url,timeout=20).decode("utf-8","replace")
        soup=BeautifulSoup(html,"html.parser")
        # RIS decision pages have the text in specific divs
...
def ai_summary(title, url, norm, rechtsgebiet):
    """Fetch decision full text, then generate 5 bullet points. Returns empty if no text available."""
    if not API_KEY:return ""
    # Fetch the actual decision text
    print(f"      Fetching text from RIS...")
    text=fetch_decision_text(url)
    if not text or len(text)<200:
        print(f"      No decision text available, skipping")
        return ""
    print(f"      Got {len(text)} chars, summarizing...")
    # Truncate to fit in context
    text=text[:4000]
    prompt=f"""Aufgabe: Erstelle genau 5 Stichpunkte zu dieser österreichischen Gerichtsentscheidung auf Deutsch. Jeder Stichpunkt beginnt mit "• " und ist ein kurzer Satz.

Die 5 Punkte sollen abdecken:
• Rechtsgebiet und betroffene Normen
• Worum es im Fall geht (Sachverhalt)
• Die zentrale Rechtsfrage
• Was das Gericht entschieden hat
• Praktische Bedeutung/Auswirkung

Entscheidung: {title}
Rechtsgebiet: {rechtsgebiet or 'nicht angegeben'}
Normen: {norm or 'nicht angegeben'}

Entscheidungstext:
{text}"""
    return ai_call(prompt, 800)[:1500]

def ai_briefing(national_headlines, international_headlines, recht_headlines):
    """Generate a morning briefing from today's headlines."""
    prompt=f"""Du bist ein erfahrener Nachrichtenredakteur. Schreibe ein kurzes, informatives Morgenbriefing auf Deutsch (ca. 8-10 Sätze). Fasse die wichtigsten Themen des Tages zusammen, gegliedert in: Österreich, International, und Recht/Justiz. Schreibe flüssig und informativ, wie ein Leitartikel. Kein Markdown, keine Aufzählungszeichen, keine Überschriften — nur Fließtext.
...
def fetch_weather_vienna():
    """Fetch today's weather for Vienna from Open-Meteo (free, no API key)."""
    print("  [Wetter] Open-Meteo Wien...")
    try:
        url="https://api.open-meteo.com/v1/forecast?latitude=48.2082&longitude=16.3738&daily=temperature_2m_max,temperature_2m_min,precipitation_sum,weathercode&current=temperature_2m,weathercode,relative_humidity_2m,wind_speed_10m&timezone=Europe/Vienna&forecast_days=1"
        ...
        w={
            ...
            "description":wmo.get(code,"Unbekannt"),
            "emoji":wmo_emoji.get(code,"\U0001f324\ufe0f"),
            "code":code
        }
        print(f"    \u2713 {w['temp_current']}\u00b0C, {w['description']}")
        return w
    except Exception as e:
...
# ═══════ MAIN ═══════
def main():
    print(f"\U0001f4f0  Morning Brief v12 \u2014 {NOW.strftime('%Y-%m-%d %H:%M UTC')}")
    if API_KEY:print(f"  \U0001f916 AI enabled (Haiku)\n")
    else:print(f"  \u26a0\ufe0f  No ANTHROPIC_API_KEY\n")
    ex={}
    if DATA.exists():
    ...
    print("\u2550\u2550 EuGH Urteile \u2550\u2550")
    r["recht_eugh"]=trim(merge_smart(ex.get("recht_eugh",[]),fetch_eugh()));print(f"  \u2192 Total: {len(r['recht_eugh'])}\n")
    ...
    # ── AI Summaries for court decisions (fetch full text, then summarize) ──
    if API_KEY:
        print("\u2550\u2550 KI-Zusammenfassungen \u2550\u2550")
        for tab in ["recht_ogh","recht_vfgh"]:
            for a in r[tab]:
                if a.get("summary") or a.get("source")!="RIS":continue
                print(f"  \U0001f916 {a['title']}...")
                s=ai_summary(a["title"],a.get("url",""),a.get("norm",""),a.get("rechtsgebiet",""))
                if s:a["summary"]=s;print(f"    \u2713 {s[:80]}...")
                else:print(f"    - no text available, skipped")
        # EuGH: keep RSS excerpt, add AI expansion as summary
        for a in r.get("recht_eugh",[]):
            if a.get("summary") or not a.get("excerpt"):continue
            print(f"  \U0001f916 {a['title']}...")
            excerpt=a["excerpt"][:3000]
            prompt=f"""Ergänze diese EuGH-Kurzmeldung um Kontext und Hintergrund. Schreibe 3-4 zusätzliche Sätze auf Deutsch die erklären: welches EU-Recht betroffen ist, warum der Fall relevant ist, und welche Auswirkungen die Entscheidung haben könnte. Antworte NUR mit den ergänzenden Sätzen als Fließtext, ohne die Originalmeldung zu wiederholen, ohne Überschrift, ohne Markdown.

Originalmeldung:
{a['title']}
{excerpt}"""
            s=ai_call(prompt,400)
            if s:a["summary"]=s[:800];print(f"    \u2713 {s[:80]}...")
        print()

    # ── AI Morning Briefing ──
    if API_KEY:
        print("\u2550\u2550 KI-Briefing \u2550\u2550")
        nat_hl="\n".join([f"- {a['title']}" for a in r.get("national",[])[:10]])
        intl_hl="\n".join([f"- {a['title']}" for a in r.get("international",[])[:10]])
        recht_hl="\n".join([f"- {a['title']}" for a in (r.get("recht_ogh",[])+r.get("recht_vfgh",[]))[:8]])
        briefing=ai_briefing(nat_hl, intl_hl, recht_hl)
        if briefing:
            r["briefing"]={"text":briefing,"date":NOW.isoformat()}
            print(f"  \u2713 Briefing: {briefing[:100]}...")
        else:
            r["briefing"]=ex.get("briefing",{})
            print(f"  \u2717 Briefing failed, keeping old")
        print()

    r["_meta"]={"last_updated":NOW.isoformat()}
    with open(DATA,"w",encoding="utf-8") as f:json.dump(r,f,ensure_ascii=False,indent=2)
    total=sum(len(r[k]) for k in r if k not in ("_meta","weather","briefing"))
    summaries=sum(1 for k in r if k not in ("_meta","weather","briefing") for a in r[k] if isinstance(a,dict) and a.get("summary"))
