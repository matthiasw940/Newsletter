#!/usr/bin/env python3
"""Morning Brief v13 — Structured briefing, bullet summaries everywhere."""
...
def ai_call(prompt,max_tokens=400):
    ...
    try:
        ...
        if err_body:print(f"      {err_body[:200]}")
        return ""
...
BULLET_PROMPT="""Aufgabe: Erstelle genau 5 Stichpunkte zu dieser Gerichtsentscheidung auf Deutsch. Jeder Stichpunkt beginnt mit "• " und ist ein vollständiger Satz.
...
def ai_summary_from_text(title,url,norm,rechtsgebiet):
    ...
    prompt=BULLET_PROMPT.format(title=title,rg=rechtsgebiet or "k.A.",norm=norm or "k.A.",text=text[:4000])
...
def ai_summary_from_excerpt(title,excerpt):
    ...
    prompt=f"""Erstelle genau 5 Stichpunkte zu dieser EuGH-Entscheidung auf Deutsch. Jeder Stichpunkt beginnt mit "• " und ist ein vollständiger Satz.

Die 5 Punkte:
• Betroffenes EU-Recht
• Sachverhalt/Vorlagefrage
• Zentrale Rechtsfrage
• Entscheidung/Schlussanträge
...
def fetch_weather_vienna():
    ...
    try:
        ...
        current=data.get("current",{});daily=data.get("daily",{});hourly=data.get("hourly",{})
        ...
        # Build hourly forecast for key hours (6,9,12,15,18,21)
        hours_forecast=[]
        ...
        print(f"    \u2713 {w['temp_current']}\u00b0C, {w['description']}, {len(hours_forecast)} hours");return w
...
def ai_briefing(r):
    """Structured briefing: news bullets + court decisions by category."""
    nat="\n".join([f"- {a['title']} ({a['source']})" for a in r.get("national",[])[:8]])
    intl="\n".join([f"- {a['title']} ({a['source']})" for a in r.get("international",[])[:8]])
    ogh="\n".join([f"- {a['title']}: {a.get('norm','')}" for a in r.get("recht_ogh",[])[:6] if a.get("source")=="RIS"])
    vfgh="\n".join([f"- {a['title']}: {a.get('norm','')}" for a in r.get("recht_vfgh",[])[:6] if a.get("source")=="RIS"])
    eugh="\n".join([f"- {a['title']}" for a in r.get("recht_eugh",[])[:4]])
    recht_news="\n".join([f"- {a['title']} ({a['source']})" for a in r.get("recht_news",[])[:4]])
    intl_law="\n".join([f"- {a['title']} ({a['source']})" for a in r.get("recht_intl",[])[:4]])
    prompt=f"""Erstelle ein strukturiertes deutsches Morgenbriefing. Verwende EXAKT dieses Format:

NACHRICHTEN

\u00d6sterreich:
• [1 Stichpunkt pro wichtige Nachricht, max 6]
...
OGH (neu ver\u00f6ffentlicht):
• [1 Stichpunkt pro Entscheidung mit GZ und Kurzinfo]

VfGH (neu ver\u00f6ffentlicht):
• [1 Stichpunkt pro Entscheidung mit GZ und Kurzinfo]

EuGH:
• [1 Stichpunkt pro Entscheidung]

Recht News:
• [1 Stichpunkt pro Nachricht]

Intl. Recht:
• [1 Stichpunkt pro Nachricht]

Regeln: Jeder Stichpunkt beginnt mit "• " und ist ein vollst\u00e4ndiger kurzer Satz. NUR neue/aktuelle Entscheidungen aufnehmen. Keine Markdown-\u00dcberschriften (#), verwende Abschnittstitel in GROSSBUCHSTABEN gefolgt von einer Leerzeile.
...
Intl. Recht:
{intl_law}"""
    return ai_call(prompt,1800)[:4000]

# ═══════ MAIN ═══════
def main():
    print(f"\U0001f4f0  Morning Brief v13 \u2014 {NOW.strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    sums=sum(1 for k in r if k not in ("_meta","weather","briefing") for a in r[k] if isinstance(a,dict) and a.get("summary"))
