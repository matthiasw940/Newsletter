...
def ris_justiz(gericht, label):
    ...
    try:
        ...
        for doc in results:
            # Actual JSON structure from RIS v2.6:
            # Data.Metadaten.Allgemein.DokumentUrl
            # Data.Metadaten.Judikatur.Geschaeftszahl.item (string or list)
            # Data.Metadaten.Judikatur.Entscheidungsdatum
            # Data.Metadaten.Judikatur.Justiz.Rechtsgebiete.item
            d = doc.get("Data", {}).get("Metadaten", {})
            allg = d.get("Allgemein", {})
            jud = d.get("Judikatur", {})

            # Geschäftszahl: can be {"item": "21Ds6/25g"} or {"item": ["1Ob1/25","2Ob2/25"]}
            gz_raw = jud.get("Geschaeftszahl", {})
            if isinstance(gz_raw, dict):
                gz_item = gz_raw.get("item", "")
                gz = gz_item if isinstance(gz_item, str) else (gz_item[0] if isinstance(gz_item, list) and gz_item else "")
            else:
                gz = str(gz_raw)
            gz = gz.strip()
            ...
            # Rechtsgebiet for category
            justiz_meta = jud.get("Justiz", {})
            rechtsgebiet = ""
            rg = justiz_meta.get("Rechtsgebiete", {})
            if isinstance(rg, dict):
                rg_item = rg.get("item", "")
                rechtsgebiet = rg_item if isinstance(rg_item, str) else (rg_item[0] if isinstance(rg_item, list) and rg_item else "")

            # Prefer GesamteEntscheidungUrl > DokumentUrl
            url = gesamt_url or doc_url
            if not url:
                url = f"https://www.ris.bka.gv.at/Ergebnis.wxe?Abfrage=Justiz&Gericht={gericht}&Geschaeftszahl={gz}"

            if not gz: continue

            cat = f"{label} ({rechtsgebiet})" if rechtsgebiet else label
            arts.append(dict(
                id=mid(url or gz),
                title=f"{label} {gz}",
...
def ris_vfgh():
    ...
    try:
        ...
        for doc in results:
            ...
            url = doc_url or f"https://www.ris.bka.gv.at/Vfgh/"
            if not gz: continue

            arts.append(dict(id=mid(url or gz),
                title=f"VfGH {gz}",
                url=url, category="VfGH", excerpt=kurz,
