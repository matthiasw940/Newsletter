...
def ai_summary(title, norm, schlagworte, rechtsgebiet, anmerkung):
    ...
    context="\n".join(parts) if parts else "Keine weiteren Metadaten verfügbar."
    prompt=f"""Du bist ein österreichischer Jurist. Fasse die folgende Gerichtsentscheidung in ca. 5 Sätzen auf Deutsch zusammen. Erkläre den Sachverhalt, die zentrale Rechtsfrage und die Entscheidung des Gerichts. Antworte NUR mit der Zusammenfassung als Fließtext, ohne Überschrift, ohne Markdown, ohne Aufzählungszeichen.
...
def main():
    ...
    # ── AI Summaries for court decisions (only final trimmed articles) ──
    if API_KEY:
        ...
        for tab in ["recht_ogh","recht_vfgh"]:
            for a in r[tab]:
                ...
                s=ai_summary(a["title"],a.get("norm",""),a.get("excerpt",""),a.get("rechtsgebiet",""),a.get("category",""))
