...
def fetch_apartments():
    """Scrape willhaben for apartments in Wien Bezirke 1-9."""
    from bs4 import BeautifulSoup
    arts=[];seen=set()
    # Single willhaben search covering Bezirke 1-9 via areaId
    # areaId for Wien 1.-9.: 900,901,902,903,904,905,906,907,908
    url="https://www.willhaben.at/iad/immobilien/mietwohnungen/wien?PRICE_TO=1200&ESTATE_SIZE_FROM=50&NUMBER_OF_ROOMS_FROM=2&rows=30&sort=1&areaId=900&areaId=901&areaId=902&areaId=903&areaId=904&areaId=905&areaId=906&areaId=907&areaId=908"
    print(f"  [Wohnung] willhaben Bez.1-9...")
    try:
        html=http_get(url,timeout=20).decode("utf-8","replace")
        soup=BeautifulSoup(html,"html.parser")
        for a in soup.find_all("a",href=True):
            ...
            bez_m=re.search(r"(\d{4})\s*Wien|Wien\s*(\d{4})",text)
            excerpt_parts=[]
            if bez_m:excerpt_parts.append(f"PLZ {bez_m.group(1) or bez_m.group(2)}")
            ...
            arts.append(dict(id=mid(href),title=text[:150],url=href,category="Mietwohnung",excerpt=" \u00b7 ".join(excerpt_parts),source="willhaben",date=NOW.isoformat(),featured=False))
        print(f"    \u2713 {len(arts)}")
    except Exception as e:print(f"    \u2717 {type(e).__name__}: {e}")
    print(f"  \u2192 {len(arts)} apartments")
    return arts[:30]
...
def main():
    ...
    if API_KEY:
        ...
        for a in r.get("recht_eugh",[]):
            ...
            if s:a["summary"]=s;ai_count+=1;print(f"    \u2713 {s[:60]}...")
        print(f"  ({ai_count} summaries generated)\n")
        # Sport briefing
