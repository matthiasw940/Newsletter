#!/usr/bin/env python3
"""Morning Brief v14 — All courts summarized, structured briefing with links."""
...
BULLET_PROMPT="""Erstelle genau 5 Stichpunkte zu dieser Gerichtsentscheidung auf Deutsch. Jeder Stichpunkt beginnt mit "\u2022 " und ist ein vollstaendiger Satz.
...
def ai_summary_from_text(title,url,norm,rechtsgebiet):
    """Fetch decision text from URL, summarize with AI. Empty if no text."""
    if not API_KEY:return ""
...
def ai_summary_from_excerpt(title,excerpt):
    """Summarize EuGH from RSS excerpt text."""
    if not API_KEY or not excerpt or len(excerpt)<50:return ""
    prompt=f"""Erstelle genau 5 Stichpunkte zu dieser EuGH-Entscheidung auf Deutsch. Jeder Stichpunkt beginnt mit "\u2022 " und ist ein vollstaendiger Satz.

\u2022 Betroffenes EU-Recht
\u2022 Sachverhalt/Vorlagefrage
\u2022 Zentrale Rechtsfrage
\u2022 Entscheidung/Schlussantraege
...
def ai_briefing(r):
    nat="\n".join([f"- {a['title']} ({a['source']}) [{a['url']}]" for a in r.get("national",[])[:8]])
    intl="\n".join([f"- {a['title']} ({a['source']}) [{a['url']}]" for a in r.get("international",[])[:8]])
    ogh="\n".join([f"- {a['title']}: {a.get('norm','')} [{a['url']}]" for a in r.get("recht_ogh",[])[:6] if a.get("source")=="RIS"])
    vfgh="\n".join([f"- {a['title']}: {a.get('norm','')} [{a['url']}]" for a in r.get("recht_vfgh",[])[:6] if a.get("source")=="RIS"])
...
Oesterreich:
[Pro Nachricht: "\u2022 " + Stichpunkt + Zeilenumbruch + "  \u2192 " + kurze Zusammenfassung (1-2 Saetze) + " [LINK:url]"]
...
[Pro Entscheidung: "\u2022 " + GZ + Kurzinfo + Zeilenumbruch + "  \u2192 " + kurze Zusammenfassung + " [LINK:url]"]

VfGH (neu veroeffentlicht):
[Gleiches Format]
...
REGELN:
- Jeder Stichpunkt mit "\u2022 " beginnen
- Nach jedem Stichpunkt eine Zeile "  \u2192 " mit 1-2 Saetze Zusammenfassung
- Links im Format [LINK:url] am Ende der Zusammenfassung
- Abschnittstitel in GROSSBUCHSTABEN
- Nur aktuelle/neue Eintraege
...
Intl. Recht: {intl_law}"""
    return ai_call(prompt,2000)[:5000]

# ═══════ MAIN ═══════
def main():
    print(f"\U0001f4f0  Morning Brief v14 \u2014 {NOW.strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    # AI Summaries — every linked RIS decision gets summarized
    if API_KEY:
        ...
        for tab in ["recht_ogh","recht_vfgh"]:
            for a in r[tab]:
                ...
                if not a.get("url") or "ris.bka.gv.at" not in a.get("url",""):continue
                print(f"  \U0001f916 {a['title']}...")
                s=ai_summary_from_text(a["title"],a["url"],a.get("norm",""),a.get("rechtsgebiet",""))
