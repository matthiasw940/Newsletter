...
BULLET_PROMPT="""Erstelle genau 5 Stichpunkte zu dieser Gerichtsentscheidung. Format:
\u2022 Normen: [betroffene Gesetze/Paragraphen, 1 Satz]
\u2022 SV: [Sachverhalt, 1-2 Saetze]
\u2022 Rechtsfrage: [zentrale Frage, 2-3 Saetze ausfuehrlich]
\u2022 Entscheidung: [rechtliche Begruendung und Ergebnis, 3-4 Saetze ausfuehrlich]
\u2022 Bedeutung: [praktische Auswirkung, 1-2 Saetze]

Die Punkte Rechtsfrage und Entscheidung sollen besonders ausfuehrlich sein. Auf Deutsch.
...
def ai_summary_from_text(title,url,norm,rechtsgebiet):
    ...
    return ai_call(prompt,1000)[:2000]
...
def fetch_apartments():
    """Scrape willhaben for apartments matching criteria."""
    print("  [Wohnung] willhaben.at...")
    try:
        from bs4 import BeautifulSoup
        # Search: Wien, Miete, 2+ Zimmer, 50+ m2, max 1200€
        url="https://www.willhaben.at/iad/immobilien/mietwohnungen/wien?&PRICE_TO=1200&ESTATE_SIZE_FROM=50&NUMBER_OF_ROOMS_FROM=2&rows=30&sort=1"
        html=http_get(url,timeout=20).decode("utf-8","replace")
        soup=BeautifulSoup(html,"html.parser")
        arts=[]
        ...
        for script in soup.find_all("script",type="application/json"):
            try:
                ...
                if isinstance(data,dict) and "searchResult" in str(data)[:200]:
                    # Try to extract listings
                    pass
            except:pass
        # Fallback: parse listing links
        for a in soup.find_all("a",href=True):
            ...
            # Extract price, size, rooms from text
            ...
            arts.append(dict(id=mid(href),title=title,url=href,category="Mietwohnung",excerpt=" · ".join(excerpt_parts) if excerpt_parts else "",source="willhaben",date=NOW.isoformat(),featured=False))
            if len(arts)>=20:break
        print(f"    \u2713 {len(arts)} listings")
        return arts
    except Exception as e:
...
def ai_briefing(r):
    ...
    ogh="\n".join([f"- {a['title']}: {a.get('norm','')} [{a['url']}]" for a in r.get("recht_ogh",[])[:8] if a.get("source")=="RIS"])
    ...
    prompt=f"""Erstelle ein ausfuehrliches deutsches Morgenbriefing. WICHTIG: Erfinde KEINE Fakten. Verwende NUR die unten angegebenen Schlagzeilen. Wenn du dir bei Details unsicher bist, bleibe allgemein.
...
Oesterreich:
[Pro Nachricht: "\u2022 " + Stichpunkt
"  \u2192 " + 2-3 Saetze Zusammenfassung basierend auf dem Titel + " [LINK:url]"]

International:
[Gleiches Format, 2-3 Saetze pro Nachricht]

RECHTSPRECHUNG

OGH (neu veroeffentlicht):
[Pro Entscheidung: "\u2022 " + GZ + Kurzinfo
"  \u2192 " + 1-2 Saetze + " [LINK:url]"]
...
REGELN:
- KEINE erfundenen Details, Orte, oder Personen hinzufuegen
- Nur die gegebenen Schlagzeilen verwenden
- Links im Format [LINK:url]
- Abschnittstitel in GROSSBUCHSTABEN
- Mindestens 8 Nachrichten-Stichpunkte, mindestens 5 Rechts-Stichpunkte
